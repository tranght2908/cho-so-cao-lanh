...
(function (A) {
  ...
  const incAssets = () => (A.db.marketAssets || []).filter(a => a.market === 'CL');
  const incCats = () => ['Điện', 'Cấp thoát nước', 'Vệ sinh', 'An ninh trật tự', 'PCCC', 'Hạ tầng', 'Khác'];
  const incAction = i => ({ tiepnhan:['inc-assign-open','Tiếp nhận & phân công'], phancong:['inc-inspect-open','Kiểm tra hiện trường'], dangxuly:['inc-work-open','Cập nhật xử lý'], chonghiemthu:['inc-accept-open','Nghiệm thu'], hoanthanh:['inc-close-open','Đóng sự cố'], dong:['inc-open','Xem hồ sơ'] }[i.state]);
  const incCan = (i, action) => A.canDo(action === 'assign' || action === 'accept' || action === 'close' ? 'su-co.phan-cong' : 'su-co.cap-nhat-xu-ly', i.market);
  ...
  A.VIEWS['su-co'] = function () { ensureIncidentV2(); const all=A.db.incidents.filter(i=>i.market==='CL'&&(!ui.incCat||i.cat===ui.incCat)); return `<div class="page-head"><div><h2>Phản ánh & sự cố</h2><p class="muted">Tiếp nhận, phân công và theo dõi xử lý sự cố tại Chợ Cao Lãnh.</p></div>${A.canDo('su-co.tao-phan-anh','CL')?'<button class="btn primary" data-act="inc-new-v2">+ Tạo phản ánh</button>':''}</div><div class="kpis"><div class="card kpi"><div class="k-label">Đang xử lý</div><div class="k-value">${all.filter(isOpen).length}</div></div><div class="card kpi"><div class="k-label">Quá hạn</div><div class="k-value" style="color:#df2225">${all.filter(late).length}</div></div></div><div class="card"><div class="card-h"><h3>Bảng theo dõi xử lý (6 trạng thái)</h3><select class="input" data-ch="inc-cat"><option value="">Mọi nhóm</option>${incCats().map(c=>`<option ${ui.incCat===c?'selected':''}>${c}</option>`).join('')}</select></div><div class="card-b"><div class="kanban inc-kanban">${ST.map(s=>{const xs=all.filter(i=>i.state===s.id);return `<div class="kcol"><h4>${s.label}<span class="muted">${xs.length}</span></h4>${xs.map(i=>{const a=incAsset(i),act=incAction(i);return `<div class="kcard ${late(i)?'late':''}"><div class="row small"><span class="muted">${i.id}</span>${late(i)?'<span class="tag danger">Quá hạn</span>':''}</div><div class="t">${U.esc(i.title)}</div><div class="small muted">${U.esc(i.cat)} · ${U.esc((A.idx.stall.get(i.stallId)||{}).code||'—')}</div>${a?`<div class="small muted">${U.esc(a.code)}</div>`:''}<div class="small muted">${incFmt(i.created)}${i.assignee?' · '+U.esc(incStaff(i)):''}</div><div class="inc-card-actions"><button class="btn sm" data-act="inc-open" data-id="${i.id}">Xem</button><button class="btn sm primary" data-act="${act[0]}" data-id="${i.id}">${act[1]}</button></div></div>`}).join('')}</div>`}).join('')}</div></div></div>`; };
  A.ACT['inc-open'] = el => { ui.incDetailTab='overview'; openIncident(A.db.incidents.find(x=>x.id===el.dataset.id)); };
  A.ACT['inc-detail-tab'] = el => { ui.incDetailTab=el.dataset.tab; openIncident(A.db.incidents.find(x=>x.id===el.dataset.id)); };
  ...
  A.ACT['inc-accept-open'] = el => { const i=A.db.incidents.find(x=>x.id===el.dataset.id);if(!i||!incCan(i,'accept'))return;incModal('Nghiệm thu kết quả xử lý',incHeader(i)+`<section class="inc-section"><h4>Trước xử lý</h4><p>${U.esc((i.inspection||{}).condition||'—')}</p></section><section class="inc-section"><h4>Kết quả xử lý</h4><p><b>Nội dung:</b> ${U.esc((i.work||{}).content||'—')}</p><p><b>Kết quả:</b> ${U.esc((i.work||{}).result||'—')}</p><p><b>Hoàn thành:</b> ${incFmt((i.work||{}).completedAt)}</p></section><section class="inc-section"><h4>Nghiệm thu</h4><div class="field"><label>Kết quả *</label><label><input type="radio" name="ia-result" value="pass" checked data-ch="inc-accept-result"> Đạt</label><label><input type="radio" name="ia-result" value="rework" data-ch="inc-accept-result"> Chưa đạt - yêu cầu xử lý lại</label></div><div class="form-grid"><div class="field"><label>Người nghiệm thu *</label><select class="input" id="ia-by">${D.STAFF.filter(s=>s.market==='CL').map(s=>`<option value="${s.id}">${U.esc(s.name)}</option>`).join('')}</select></div><div class="field"><label>Thời gian nghiệm thu *</label><input class="input" type="datetime-local" id="ia-at" value="${incNow()}"></div></div><div class="field"><label>Ý kiến nghiệm thu</label><textarea class="input" id="ia-note" rows="2"></textarea></div><div class="field" id="ia-rework-wrap" hidden><label>Yêu cầu xử lý lại *</label><textarea class="input" id="ia-rework" rows="2"></textarea></div></section>`,`<button class="btn" data-act="close">Hủy</button><button class="btn primary" data-act="inc-accept-save" data-id="${i.id}">Xác nhận nghiệm thu</button>`); };
  A.CH['inc-accept-result'] = el => { const wrap=A.$('#ia-rework-wrap'); if(wrap) wrap.hidden=el.value!=='rework'; };
  A.ACT['inc-accept-save'] = el => {const i=A.db.incidents.find(x=>x.id===el.dataset.id),r=(A.$('input[name="ia-result"]:checked')||{}).value,by=A.$('#ia-by').value,at=A.$('#ia-at').value,re=A.$('#ia-rework').value.trim();if(!i||!incCan(i,'accept'))return;if(!by||!at||(r==='rework'&&!re)){U.toast(r==='rework'?'Vui lòng nhập yêu cầu xử lý lại.':'Vui lòng nhập người và thời gian nghiệm thu.');return;}i.acceptance={result:r,by,at,note:A.$('#ia-note').value.trim(),rework:re};i.state=r==='pass'?'hoanthanh':'dangxuly';incHistory(i,r==='pass'?'Nghiệm thu đạt':'Nghiệm thu chưa đạt',r==='pass'?'':'Yêu cầu xử lý lại: '+re);A.save();A.closeModal();A.render();U.toast(r==='pass'?'Đã nghiệm thu đạt.':'Đã trả lại để xử lý.');};
  A.ACT['inc-close-open'] = el => {const i=A.db.incidents.find(x=>x.id===el.dataset.id);if(!i||!incCan(i,'close'))return;incModal('Đóng sự cố',incHeader(i)+`<section class="inc-section"><div class="inc-check">✓ Đã xử lý</div><div class="inc-check">✓ Đã nghiệm thu đạt</div><dl class="kv"><dt>Người xử lý</dt><dd>${U.esc(incStaff(i))}</dd><dt>Người nghiệm thu</dt><dd>${U.esc(U.staffName((i.acceptance||{}).by))}</dd><dt>Hoàn thành</dt><dd>${incFmt((i.work||{}).completedAt)}</dd></dl><div class="field"><label>Ghi chú khi đóng</label><textarea class="input" id="ic-note" rows="2"></textarea></div></section>`,`<button class="btn" data-act="close">Hủy</button><button class="btn primary" data-act="inc-close-save" data-id="${i.id}">Đóng sự cố</button>`);};
  A.ACT['inc-close-save'] = el => {const i=A.db.incidents.find(x=>x.id===el.dataset.id);if(!i||!incCan(i,'close'))return;i.closeInfo={at:incNow(),note:A.$('#ic-note').value.trim()};i.state='dong';incHistory(i,'Đóng sự cố',i.closeInfo.note);A.save();A.closeModal();A.render();U.toast('Đã đóng sự cố.');};
  A.ACT['inc-new-v2'] = () => { if(!A.canDo('su-co.tao-phan-anh','CL'))return;ui.incImageDraft={};const stalls=A.db.stalls.filter(s=>s.market==='CL');incModal('Tạo phản ánh / sự cố',`<section class="inc-section"><div class="form-grid"><div class="field"><label>Nguồn</label><input class="input" value="Nhập tại Ban Quản lý" disabled></div><div class="field"><label>Nhóm *</label><select class="input" id="in2-cat">${incCats().map(c=>`<option>${c}</option>`).join('')}</select></div><div class="field"><label>Điểm KD</label><select class="input" id="in2-stall"><option value="">Không xác định</option>${stalls.map(s=>`<option value="${s.id}">${s.code}</option>`).join('')}</select></div><div class="field"><label>Tài sản liên quan</label><select class="input" id="in2-asset"><option value="">Không xác định / Không liên quan</option>${incAssets().map(a=>`<option value="${a.id}">${U.esc(a.code)} · ${U.esc(a.name)}</option>`).join('')}</select></div></div><div class="field"><label>Tiêu đề *</label><input class="input" id="in2-title"></div><div class="field"><label>Nội dung *</label><textarea class="input" id="in2-desc" rows="3"></textarea></div></section><section class="inc-section"><h4>Ảnh <span class="muted small">(không bắt buộc)</span></h4>${incImageInput('report')}</section>`,`<button class="btn" data-act="close">Hủy</button><button class="btn primary" data-act="inc-new-v2-save">Tạo phản ánh</button>`);};
  A.ACT['inc-new-v2-save'] = () => {const title=A.$('#in2-title').value.trim(),desc=A.$('#in2-desc').value.trim(),stall=A.idx.stall.get(A.$('#in2-stall').value),cat=A.$('#in2-cat').value;if(!title||!desc){U.toast('Vui lòng nhập tiêu đề và nội dung.');return;}const id='SC-'+U.pad(101+A.db.incidents.length,4),created=incNow(),due=new Date(U.today());due.setDate(due.getDate()+(cat==='Điện'||cat==='PCCC'?1:3));const i={id,market:'CL',stallId:stall&&stall.id,traderId:stall&&stall.traderId,assetId:A.$('#in2-asset').value||null,cat,title,desc,source:'Nhập tại Ban Quản lý',state:'tiepnhan',created,deadline:due.toISOString().slice(0,16),assignee:null,rating:null,escalated:false,images:{report:incDraftNames('report'),inspection:[],work:[]},history:[{at:created,action:'Tiếp nhận phản ánh',detail:'Nguồn: Nhập tại Ban Quản lý'}],log:[]};A.db.incidents.push(i);A.save();A.closeModal();A.render();U.toast('Đã tạo '+id);};
