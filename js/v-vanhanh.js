...
(function (A) {
  ...
  if (!ui.permRole) ui.permRole = 'bql';
  ...
  A.VIEWS['su-co'] = function () {
    ...
    <div class="card"><div class="card-h"><h3>Bảng theo dõi xử lý (6 trạng thái)</h3>
      ...
      ${A.PERM.canAction(ui.role, 'su-co.tao-phan-anh') ? '<button class="btn primary" data-act="inc-new">+ Tạo phản ánh</button>' : ''}</div>
  ...
  A.ACT['inc-open'] = el => {
    ...
    const canPhanCong = A.PERM.canAction(ui.role, 'su-co.phan-cong');
    const canVuotCap = A.PERM.canAction(ui.role, 'su-co.vuot-cap');
    const canChiDao = A.PERM.canAction(ui.role, 'su-co.chi-dao');
    const canChuyenTT = A.PERM.canAction(ui.role, 'su-co.chuyen-trang-thai');
  ...
  function accRows() {
    ...
    return A.ACCOUNTS.list().filter(a =>
      ...
      (!f.market || (a.marketScopes || []).includes(f.market)) &&
  ...
  function accDrawerHtml(a) {
    const canEdit = A.PERM.canAction(ui.role, 'tai-khoan.sua');
  ...
  function renderAccForm() {
    ...
    const canAssign = isNew || A.PERM.canAction(ui.role, 'tai-khoan.gan-quyen');
    ...
    A.modal(A.mHead(isNew ? 'Thêm tài khoản mới' : 'Sửa tài khoản') + `<div class="modal-b"><div class="form-grid">
      ...
      <div class="field"><label>Phạm vi chợ</label><select class="input" data-ch="af-scope" ${dis}><option value="ALL" ${(d.marketScopes && d.marketScopes[0]) === 'ALL' ? 'selected' : ''}>Toàn hệ thống</option>${D.MARKETS.map(m => `<option value="${m.id}" ${(d.marketScopes && d.marketScopes[0]) === m.id ? 'selected' : ''}>${m.short}</option>`).join('')}</select></div>
    ...
    ${!canAssign ? '<div class="note" style="margin-top:12px">Bạn không có quyền gán vai trò / phạm vi chợ nên 2 trường này đang bị khoá.</div>' : ''}
    </div>
  ...
  A.VIEWS['tai-khoan'] = function () {
    const canCreate = A.PERM.canAction(ui.role, 'tai-khoan.tao-moi');
    const canEdit = A.PERM.canAction(ui.role, 'tai-khoan.sua');
    const canToggle = A.PERM.canAction(ui.role, 'tai-khoan.khoa-mo-khoa');
    ...
    <div class="card"><div class="card-b row" style="padding-top:14px;flex-wrap:wrap">
      ...
      <select class="input" data-ch="acc-market"><option value="">Chợ / phạm vi: Tất cả</option><option value="ALL" ${f.market === 'ALL' ? 'selected' : ''}>Toàn hệ thống</option>${D.MARKETS.map(m => `<option value="${m.id}" ${f.market === m.id ? 'selected' : ''}>${m.short}</option>`).join('')}</select>
  ...
  A.ACT['acc-new'] = () => {
    ui.accForm = { id: null, code: '', fullName: '', phone: '', accountType: A.ACCOUNTS.ACCOUNT_TYPES[3], roleIds: [], organization: '', marketScopes: ['ALL'], status: 'active' };
    renderAccForm();
  };
  A.ACT['acc-edit'] = el => {
    const a = A.ACCOUNTS.get(el.dataset.id);
  ...
  A.CH['af-scope'] = el => { ui.accForm.marketScopes = [el.value]; };
  ...
  A.ACT['acc-form-save'] = () => {
    ...
    const patch = { code: d.code.trim(), fullName: d.fullName.trim(), phone: (d.phone || '').trim(), accountType: d.accountType, roleIds: d.roleIds, organization: (d.organization || '').trim(), marketScopes: d.marketScopes, status: d.status };
    if (d.id) {
  ...
  function scopeLabel(r) {
    if (r.scope === 'self') return 'Chỉ dữ liệu của mình';
    if (r.scope === 'market') return 'Chợ ' + (U.market(r.market) ? U.market(r.market).short : (r.market || '?'));
    return 'Tất cả chợ';
  }
  function slugify(s) {
  ...
  function permsMatrixHtml(role) {
    ...
    const groups = [];
    A.PERM.catalog().forEach(p => {
      let g = groups.find(x => x.name === p.group);
      if (!g) { g = { name: p.group, items: [] }; groups.push(g); }
      g.items.push(p);
    });
    return groups.map(g => `<div style="margin-bottom:14px"><div class="small" style="font-weight:600;margin-bottom:6px">${U.esc(g.name)}</div>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:4px 14px">
        ${g.items.map(p => `<label class="small" style="display:flex;gap:6px;align-items:center;padding:3px 0">
          <input type="checkbox" data-ch="perm-toggle" data-role="${role.id}" data-key="${p.key}" ${granted.has(p.key) ? 'checked' : ''}>
          ${U.esc(p.label)}${p.kind === 'action' && A.menuItem(p.screenId) ? ` <span class="tag">${U.esc(A.menuItem(p.screenId).label)}</span>` : ''}</label>`).join('')}
      </div></div>`).join('');
  }
  function renderRoleForm() {
    ...
    A.modal(A.mHead(d.id ? 'Sửa vai trò' : 'Thêm vai trò mới') + `<div class="modal-b"><div class="form-grid">
      ...
      <div class="field"><label>Phạm vi dữ liệu</label><select class="input" data-ch="rf-scope">
        <option value="all" ${d.scope === 'all' ? 'selected' : ''}>Tất cả chợ</option>
        <option value="market" ${d.scope === 'market' ? 'selected' : ''}>Theo 1 chợ cụ thể</option>
        <option value="self" ${d.scope === 'self' ? 'selected' : ''}>Chỉ dữ liệu của mình (tự phục vụ)</option>
      </select></div>
      ${d.scope === 'market' ? `<div class="field"><label>Chợ</label><select class="input" data-ch="rf-market">${D.MARKETS.map(m => `<option value="${m.id}" ${d.market === m.id ? 'selected' : ''}>${m.short}</option>`).join('')}</select></div>` : ''}
    </div>
    ...
    ${d.scope === 'self' ? '<div class="note info" style="margin-top:12px">Vai trò phạm vi "Chỉ dữ liệu của mình" sẽ tự động vào thẳng Mini app tiểu thương khi được chọn, giống vai trò Tiểu thương hiện tại.</div>' : ''}
    </div>
  ...
  function settingsVaitroHtml() {
    const selRole = A.PERM.role(ui.permRole) || A.PERM.role('bql');
    return `<div class="card"><div class="card-h"><h3>Vai trò và phân quyền</h3><span class="small muted">Nguyên tắc tối thiểu quyền hạn</span><button class="btn sm primary" data-act="role-new">+ Thêm vai trò</button></div><div class="card-b">
      ${U.table([{ t: 'Vai trò' }, { t: 'Mô tả' }, { t: 'Phạm vi dữ liệu' }, { t: 'Trạng thái' }, { t: '' }], A.PERM.roles().map(r => `<tr>
        ...
        <td class="nowrap">
          ...
          <button class="btn sm" data-act="role-edit" data-id="${r.id}">Sửa</button>
          <button class="btn sm ${r.active ? 'danger' : ''}" data-act="role-toggle" data-id="${r.id}">${r.active ? 'Vô hiệu hoá' : 'Kích hoạt'}</button>
          ${!r.builtin ? `<button class="btn sm danger" data-act="role-del" data-id="${r.id}">Xoá</button>` : ''}
        </td></tr>`))}</div></div>
  ...
  function settingsNhatkyHtml() {
    ...
    return `<div class="card"><div class="card-h"><h3>Nhật ký kiểm toán</h3><button class="btn danger" data-act="reset">↺ Đặt lại dữ liệu mẫu</button></div><div class="card-b">
  ...
  A.ACT['role-new'] = () => { ui.roleForm = { id: null, name: '', desc: '', scope: 'all', market: null }; renderRoleForm(); };
  A.ACT['role-edit'] = el => {
    const r = A.PERM.role(el.dataset.id);
    if (!r) return;
    ui.roleForm = { id: r.id, name: r.name, desc: r.desc, scope: r.scope, market: r.market };
    renderRoleForm();
  };
  A.CH['rf-name'] = el => { ui.roleForm.name = el.value; };
  A.CH['rf-desc'] = el => { ui.roleForm.desc = el.value; };
  A.CH['rf-market'] = el => { ui.roleForm.market = el.value; };
  A.CH['rf-scope'] = el => {
    ui.roleForm.scope = el.value;
    if (el.value === 'market' && !ui.roleForm.market) ui.roleForm.market = 'CL';
    if (el.value !== 'market') ui.roleForm.market = null;
    renderRoleForm();
  };
  A.ACT['role-form-save'] = () => {
    const d = ui.roleForm;
    if (!d.name || !d.name.trim()) { U.toast('Vui lòng nhập tên vai trò'); return; }
    if (d.scope === 'market' && !d.market) { U.toast('Vui lòng chọn chợ'); return; }
    const patch = { name: d.name.trim(), desc: (d.desc || '').trim(), scope: d.scope, market: d.scope === 'market' ? d.market : null, selfService: d.scope === 'self' };
    if (d.id) {
  ...
  A.ACT['role-del-ok'] = el => {
    const r = A.PERM.role(el.dataset.id), name = r ? r.name : '';
    A.PERM.removeRole(el.dataset.id);
    if (ui.permRole === el.dataset.id) ui.permRole = 'bql';
