...
(function (A) {
  ...
  A.ACT.trader = (el, e) => {
    ...
    A.modal(A.mHead(U.esc(t.name) + ' · ' + t.id) + `<div class="modal-b"><div class="grid g2">
      ...
      </div><div class="modal-f">${A.canDirectCollect(t.market) && U.traderDebt(t.id) ? `<button class="btn primary" data-act="pay-open" data-id="${t.id}">💵 Thu tiền mặt</button>` : ''}<button class="btn" data-act="close">Đóng</button></div>`, true);
  };
